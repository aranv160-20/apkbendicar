...
class Tbl_userController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				tbl_user.id_user LIKE ? OR 
				tbl_user.username LIKE ? OR 
				tbl_user.password LIKE ? OR 
				tbl_user.level LIKE ? OR 
				tbl_user.email LIKE ? OR 
				tbl_user.foto LIKE ? OR 
				tbl_user.nama LIKE ?
			)";
			$search_params = array(
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("tbl_user.id_user", ORDER_TYPE);
		}
		if($fieldname){
		...
		$page_title = $this->view->page_title = "Tbl User";
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("tbl_user.id_user", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = "View  Tbl User";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("username","password","level","email","foto","nama");
		...
		$page_title = $this->view->page_title = "Add New Tbl User";
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id_user","username","foto","nama");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'username' => 'required',
			...
			if($this->validated()){
				$db->where("tbl_user.id_user", $rec_id);;
		...
		$page_title = $this->view->page_title = "Edit  Tbl User";
		if(!$data){
			$this->set_page_error();
		}
	...
	function delete($rec_id = null){
		...
		$db->where("tbl_user.id_user", $arr_rec_id, "in");
